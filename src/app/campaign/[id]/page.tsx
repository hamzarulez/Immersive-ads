...
export default async function CampaignPage({ params }: { params: { id: string } }) {
    ...
    const campaignId = parseInt(params.id, 10); // Ensure the ID is a number

    // --- UPDATED DATA FETCHING: Call the RPC function ---
    // We expect only one result, so we use .single()
    const { data: campaign } = await supabase
        .rpc('get_campaign_details', { p_campaign_id: campaignId })
        .single();

    // If the RPC call returns no data, show a 404 page
    if (!campaign) {
        notFound();
    }
                        ...
                        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-8 sticky top-32">
                             <div className="flex items-center gap-4 mb-6">
                                {campaign.poster_avatar_url ? (
                                    <img src={campaign.poster_avatar_url} alt={campaign.brand_name || 'Brand'} className="w-16 h-16 rounded-lg object-cover border-2 border-neutral-700" />
                                ) : (
                            ...
                            {campaign.poster_website_url && (
                                <a href={campaign.poster_website_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-pink-400 hover:underline">
                                    <Globe size={16} />
                                    <span>Visit Website</span>
                                </a>
                            )}
                            
                            <Button className="w-full mt-8 bg-indigo-500 hover:bg-indigo-600 rounded-full py-3 font-semibold">
                                <Mail size={16} className="mr-2" /> Contact Brand
                            </Button>
                        </div>
                    </aside>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {campaign.platforms?.map((platform: string) => (
                                            <span key={platform} className="bg-neutral-700 text-neutral-300 text-xs font-semibold px-3 py-1 rounded-full">{platform}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
